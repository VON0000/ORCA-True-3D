...
let pp_v3 (v : V3.t) =
  Printf.sprintf "(%.3f, %.3f, %.3f)" v.x v.y v.z
...
let run_demo () =
  ...
  Printf.fprintf csv
    "step,t,uav_x,uav_y,uav_z,uav_vx,uav_vy,uav_vz,dyn_x,dyn_y,dyn_z,stat_x,stat_y,\
     stat_z,d_min,rpz,rpf,target_x,target_y,target_z\n";
  ...
  let write_row ~step ~t ~(uav : Avoid.state) ~(dyn : Avoid.obstacle) ~d_min =
    Printf.fprintf csv
      "%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,\
       %.6f,%.6f,%.6f,%.6f,%.6f\n"
      step t uav.Avoid.pos.x uav.Avoid.pos.y uav.Avoid.pos.z uav.Avoid.vel.x
      uav.Avoid.vel.y uav.Avoid.vel.z dyn.Avoid.pos.x dyn.Avoid.pos.y dyn.Avoid.pos.z
      static_obs.Avoid.pos.x static_obs.Avoid.pos.y static_obs.Avoid.pos.z d_min
      params.Avoid.rpz params.Avoid.rpf target.x target.y target.z
  in
  let rec loop k (uav : Avoid.state) (dyn : Avoid.obstacle) =
    if k > max_steps then (
      ...
      if k mod 20 = 0 then (
        Printf.printf
          "t=%5.2fs pos=%s vel=%s d_min=%.3f\n%!"
          t (pp_v3 uav_next.pos) (pp_v3 uav_next.vel)
          d_min );
      if V3.distance uav_next.pos target < 0.25 then
        (
          close_csv ();
          Printf.printf
            "Reached target at t=%.2fs, pos=%s\n%!"
            t (pp_v3 uav_next.pos) )
  ...
  let dyn0 : Avoid.obstacle =
    {
      Avoid.pos = V3.make (-1.5) 1.5 2.5;
      vel = V3.make 0.10 (-0.12) 0.0;
    }
  in
