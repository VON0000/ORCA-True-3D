...
let cone_geometry rpz (p_b : V3.t) =
  ...
  if d <= rpz +. eps then None
    ...
    Some { d; rvo; dvo; theta_az; theta_el }
...
let delta_h h x =
  if x < 0.0 then 1.0
  else if x < h then 1.0
  else if x <= 1.0 then
    0.5 *. ((cos (pi *. (x -. h) /. max eps (1.0 -. h))) +. 1.0)
  else 0.0

let sigma_fun x = x /. sqrt (1.0 +. (x *. x))

let theta_fun a b x =
  let e = sqrt (abs_float (a -. b) /. max eps (4.0 *. a *. b)) in
  0.5 *. (((a +. b) *. sigma_fun (x +. e)) +. (a -. b))

let sigma_norm eps_sigma x =
  (sqrt (1.0 +. (eps_sigma *. x *. x)) -. 1.0) /. eps_sigma

let theta_pf params lambda =
  let bump = delta_h params.h (lambda /. max eps params.cpf) in
  bump *. theta_fun params.a params.b (lambda -. params.rpf)

let radial_velocity params (p_b : V3.t) =
  let d = V3.norm p_b in
  if d > params.rpf || d < eps then V3.zero
  else
    let mag =
      params.kpf *. theta_pf params (sigma_norm params.eps_sigma d)
    in
    (* Repulsion direction: from obstacle center to UAV. *)
    V3.((-.mag) * V3.normalize p_b)
  ...
  | Some g ->
      ...
      let phi_target =
        if abs_float target_y < eps then
          clamp (-0.5 *. pi +. 1e-6) (0.5 *. pi -. 1e-6)
            (if target_z >= 0.0 then 0.5 *. pi else -.0.5 *. pi)
        else atan (target_z /. target_y)
      in
      let lo = clamp (-0.5 *. pi +. 1e-6) (0.5 *. pi -. 1e-6) (phi_target -. params.phi_window) in
      let hi = clamp (-0.5 *. pi +. 1e-6) (0.5 *. pi -. 1e-6) (phi_target +. params.phi_window) in
      ...
      let pick_for_phi phi =
        ...
        let boundary =
          Array.fold_left
            (fun acc tp ->
              let b_vo_c =
                V3.make g.dvo (g.rvo *. cos tp) (g.rvo *. sin tp)
              in
              let b_vo_b = V3.(rotate_c_to_b g.theta_az g.theta_el b_vo_c + obs.vel) in
...
let desired_velocity params ~target (uav : state) (obstacles : obstacle list) =
  ...
  | Some (obs_near, d_near) ->
      let outward =
        if d_near < eps then V3.zero
        else V3.normalize V3.(uav.pos - obs_near.pos)
      in
      let v_pf =
        List.fold_left
          (fun acc obs -> V3.(acc + radial_velocity params (obs.pos - uav.pos)))
          V3.zero obstacles
      in
      ...
      let v_vo_raw =
        if need_vo then
          ...
          | Some v ->
              V3.(0.55 * v_pref + (0.45 * v))
              |> limit_speed params.max_speed
          | None -> v_pref
        else v_pref
      in
      let v_vo =
        if V3.norm V3.(v_vo_raw - obs_near.vel) < (0.25 *. params.max_speed) then
          V3.(v_vo_raw + ((0.5 *. params.max_speed) * outward))
          |> limit_speed params.max_speed
        else v_vo_raw
      in
      let v_d =
        if d_near <= params.rpz then
          V3.(v_pf + ((0.8 *. params.max_speed) * outward))
        else if d_near <= params.rpf then V3.(v_vo + v_pf)
        else v_vo
      in
      limit_speed params.max_speed v_d
